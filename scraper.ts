...
export async function launchAndLogin(
  ...
  try {
    ...
    await page
      ...
      .catch(() => {}); // .catch → don't crash if screenshot also fails
 ...
 */
export async function scrapeOpenIssues(
  page: Page,
  projectValue: string,
): Promise<IssueRow[]> {
  ...
  const issues = await page.evaluate(() => {
    ...
    for (const tbl of allTables) {
      ...
      if (
        ...
        firstCell.textContent.includes("Viewing Issues")
      ) {
    ...
    for (const row of rows) {
      ...
      // Filter out issues where the assignee contains "[SW2] [SW1] [korean]" (case-insensitive check)
      const assigneeLower = (assignee || "").toLowerCase();

      if (
        assigneeLower.includes("[sw2]") ||
        assigneeLower.includes("[sw1]") ||
        assignee.includes("[제조기술]")
      ) {
        continue;
      }
...
export async function closeBrowser(page: Page): Promise<void> {
  try {
    ...
    /* page may already be closed — ignore the error */
  }
